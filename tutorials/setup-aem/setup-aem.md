...
## Prerequisites
- If you are completing the Get started with SAP Integration Suite, advanced event mesh mission, you should have access to an SAP BTP enterprise account.
...
## Intro
In this tutorial, we will setup AEM, start interacting with it and we will get familiar with some key parts of the product.
...
3. Once the instance is created, we can click on the instance name to access the AEM instance.
...
> For simplicity purposes we will refer to the event broker service as `AMER-USEast-Broker` in this tutorial. In this case, the name of the event broker service reflects the region where the broker is located but we can name it whatever we want.
...
In our case, we have only one event broker services, the event broker service we've set up.
...
As we learned in the previous section, an event mesh is composed of multiple event brokers that can span different regions and data centers. An event broker can only belong to a single mesh and the communication between event broker services in a mesh is powered by Dynamic Message Routing.event-broker-configuration
...
In this tutorial, we explored the AEM instance that we created. We navigated through the different sections of the start page, the Cluster Manager, the Event Broker services, the Mesh Manager and the Insights section. We also create an event broker service.
